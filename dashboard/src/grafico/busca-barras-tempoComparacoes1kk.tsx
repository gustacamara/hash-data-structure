...
import { comparacoesUm } from "@/data/data"
...
function calculaVariacao(): number {
  let media: number = 0
  comparacoesUm.map((value) => {
    media += value.ms
  })
  media = media / comparacoesUm.length
  media = Math.round(media)
  return media
}

export function BarrasTempoComparacoesUmKK() {
  return (
    <Card className="grid grid-cols-2-col col-span-3 gap-4">
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="h-[240px] w-full">
          <BarChart accessibilityLayer data={comparacoesUm}>
            ...
            <XAxis
              ...
              tickMargin={10}
              axisLine={false}
              tickFormatter={(_, value) => (comparacoesUm[value]?.legenda ?? "").slice(0, 4)}
            />
            <ChartTooltip
              cursor={false}
